...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      console.log("/workspaces/:workspace/channels")
      console.log(req)
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const channel = await Channel.create(
        {
          name: "공부방",
          WorkspaceId: req.user.dataValues.id,
        },
        {
          transaction: t,
        }
...
router.post("/users", isNotLoggedIn, async (req, res, next) => {
  try {
    ...
    const hapse = await Workspace.findOne({ where: { id: 1 } });
    const channel = await Channel.findOne({ where: { id: 1 } });
    await hapse.addMembers(user);
    await channel.addMembers(user);
